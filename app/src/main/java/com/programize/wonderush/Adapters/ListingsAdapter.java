...
public class ListingsAdapter extends RecyclerView.Adapter<ListingsAdapter.MainViewHolder> {
    ...
    //CONSTRUCTOR
    public ListingsAdapter(Context mContext,JSONArray dataset) {
    ...
    @Override
    public ListingsAdapter.MainViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View v = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.listview_listings_item, viewGroup, false);
        return new MainViewHolder(v);
    }

    @Override
    public void onBindViewHolder(final ListingsAdapter.MainViewHolder viewHolder, int i) {
        String id ="";
        String img_url ;
        int category_id ;
        ...
        try {
            //TODO PUT VALID VALUES FROM JSON
            id = mDataset.getJSONObject(i).getString("id") ;

            img_url = Definitions.APIdomain + mDataset.getJSONObject(i).getString("experience_image") ;
//            category_id = mDataset.getJSONObject(i).getJSONArray("category_ids").getInt(0);
            experience_category = mDataset.getJSONObject(i).getJSONArray("experience_categories").getJSONObject(0).getString("name");

            viewHolder.txt_locationname.setText(mDataset.getJSONObject(i).getString("address_line_2"));
            viewHolder.txt_name.setText(mDataset.getJSONObject(i).getString("name").toUpperCase());
            viewHolder.txt_calendar.setText(mActions.getDays(mDataset.getJSONObject(i).getJSONArray("days_of_week")));

            if(mDataset.getJSONObject(i).getBoolean("bucketed"))
            {
                viewHolder.img_bucket.setImageResource(R.drawable.icon_bucket_on);
//                Picasso.with(mContext).load(R.drawable.icon_bucket_on).resize(40, 40).into(viewHolder.img_bucket);
            }
            else
            {
                viewHolder.img_bucket.setImageResource(R.drawable.icon_bucket_off);
//                Picasso.with(mContext).load(R.drawable.icon_bucket_off).resize(40, 40).into(viewHolder.img_bucket);
            }
//            Picasso.with(mContext).load(img_url).resize(600, 400).onlyScaleDown().memoryPolicy(MemoryPolicy.NO_STORE).networkPolicy(NetworkPolicy.NO_STORE).into(viewHolder.img_background);
...
//            Picasso.with(mContext).load(mActions.getCategorySmallImage2(experience_category)).into(viewHolder.img_type);
            viewHolder.img_type.setImageResource(mActions.getCategorySmallImage2(experience_category));
...
//                Toast.makeText(mContext, "Position: " + finalPosition + "", Toast.LENGTH_SHORT).show();
    ...
    private void eventbucket(final String id, final int position, final ListingsAdapter.MainViewHolder viewHolder)
    {
        String path = "";
        try {
            if(mDataset.getJSONObject(position).getBoolean("bucketed"))
            {
...
//                            mActions.displayToast(mContext, "Bucket change for id: "+id+" was successful !");
                            ...
                            try {
                                if(mDataset.getJSONObject(position).getBoolean("bucketed"))
                                {
                                    mDataset.getJSONObject(position).put("bucketed", false);
                                    viewHolder.img_bucket.setImageResource(R.drawable.icon_bucket_off);
//                                    Picasso.with(mContext).load(R.drawable.icon_bucket_off).resize(40, 40).into(viewHolder.img_bucket);
                                }
                                else
                                {
                                    mDataset.getJSONObject(position).put("bucketed", true);
                                    viewHolder.img_bucket.setImageResource(R.drawable.icon_bucket_on);
//                                    Picasso.with(mContext).load(R.drawable.icon_bucket_on).resize(40, 40).into(viewHolder.img_bucket);
                                }
                            } catch (JSONException e) {
