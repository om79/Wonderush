...
//        this.mainActionView.setOnClickListener(new ActionViewClickListener());
        this.mainActionView.setOnTouchListener(new ActionViewTouchListener());
    ...
    public void close(boolean animated) {
        if(rl.getWindowToken()!=null) {
            removeViewFromCurrentContainer(rl);
        }
        // If animations are enabled and we have a MenuAnimationHandler, let it do the heavy work
        if(animated && animationHandler != null) {
            if(animationHandler.isAnimating()) {
    ...
    public void myTerminateFunc(boolean animated) {
        if(rl.getWindowToken()!=null) {
            removeViewFromCurrentContainer(rl);
        }
        // If animations are enabled and we have a MenuAnimationHandler, let it do the heavy work
        ...
        // do not forget to specify that the menu is now closed.
        open = false;
        removeViewFromCurrentContainer(mainActionView);
        if(stateChangeListener != null) {
            stateChangeListener.onMenuClosed(this);
        }
    }
    ...
    public View getActivityContentView() {
        //TODO get coords
        if(!prefs.contains("initialX") &&(mainActionView.getX() !=0.0))
        {
            SharedPreferences.Editor editor = prefs.edit();
            editor.putFloat("initialX", mainActionView.getX());
            editor.putFloat("initialY", mainActionView.getY());
            editor.putFloat("paramsX", mainActionView.getX());
            editor.putFloat("paramsY", mainActionView.getY());
            editor.apply();
        }

        if((firstTime)&&(mainActionView.getX() !=0.0))
        {
            mainActionView.setX(prefs.getFloat("paramsX", -1));
            mainActionView.setY(prefs.getFloat("paramsY", -1));
    ...
    public class ActionViewClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if(!open)
            {
                ...
                v.animate()
                        .x(prefs.getFloat("paramsX", -1))
                        .y(prefs.getFloat("paramsY", -1))
                        .setDuration(300)
                        .setListener(new Animator.AnimatorListener() {
                            ...
                            public void onAnimationEnd(Animator animator) {
                                mainActionView.setOnTouchListener(new ActionViewTouchListener());
                            }
    ...
    public static class Builder {
        ...
        public Builder(Context context, boolean systemOverlay) {
            ...
            //TODO my info
            prefs = context.getSharedPreferences("test.test", Context.MODE_PRIVATE);
            rl = new RelativeLayout(context);
            rl.setBackgroundColor(Color.parseColor("#50000000"));
        }
...
//                    dY = view.getY() - event.getRawY();
                ...
                case MotionEvent.ACTION_UP:
                    if ((Math.abs(initialTouchX - event.getRawX()) < 11) && (Math.abs(initialTouchY - event.getRawY()) < 11))
                    {
                        if(!open) {

                            if(rl.getWindowToken()==null) {
                                addViewToCurrentContainer(rl);
                            }
                            ...
                            mainActionView.setOnTouchListener(null);
                            view.animate()
                                    .x(prefs.getFloat("initialX", -1))
                                    .y(prefs.getFloat("initialY", -1))
                                    .setDuration(300)
                                    .setListener(new Animator.AnimatorListener() {
                                        @Override
                                        public void onAnimationStart(Animator animator) {
...
//                            .start();
                    ...
                    if ((newX <= 0 || newX >= sW-view.getWidth()) || (newY <= 0 || newY >= sH-view.getHeight()-92))
                        break;
                    ...
                    SharedPreferences.Editor editor = prefs.edit();
                    editor.putFloat("paramsX", newX);
                    editor.putFloat("paramsY", newY);
                    editor.apply();
                    break;
